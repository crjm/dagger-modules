...
func (r *Vite) UnmarshalJSON(bs []byte) error {
	var concrete struct{}
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (_ any, err error) {
	...
	case "Vite":
		...
		case "Source":
			...
			if inputArgs["dirPath"] != nil {
				err = json.Unmarshal([]byte(inputArgs["dirPath"]), &dirPath)
				if err != nil {
					panic(fmt.Errorf("%s: %w", "failed to unmarshal input arg dirPath", err))
				}
			}
			return (*Vite).Source(&parent, dirPath), nil
		case "InstallDeps":
			var parent Vite
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
			...
			return (*Vite).InstallDeps(&parent, ctx, dirPath)
		case "Build":
			var parent Vite
			err = json.Unmarshal(parentJSON, &parent)
			if err != nil {
	...
	case "":
		return dag.Module().
			WithObject(
				dag.TypeDef().WithObject("Vite").
					WithFunction(
						dag.Function("Source",
							dag.TypeDef().WithObject("Container")).
							WithDescription("prints the contents of the vite project directory").
							WithArg("dirPath", dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("InstallDeps",
							dag.TypeDef().WithKind(StringKind)).
							WithDescription("build the vite project").
							WithArg("dirPath", dag.TypeDef().WithObject("Directory"))).
					WithFunction(
						dag.Function("Build",
							dag.TypeDef().WithKind(VoidKind).WithOptional(true)).
							WithArg("dirPath", dag.TypeDef().WithObject("Directory")))), nil
	default:
